...
type IConfig<T extends readonly string[]> = {
  [K in T[number]]?: RecognizerOptions;
}& {
  customEvents?: {
    [key: string]: {
      type: T[number];
...
let config:IConfig<typeof gestures> = {};

/**
 * 初始化hammer配置,参照hammer官网相关set配置
 * 格式:{
    pan: { enable: true },
      // 自定义指令
      customEvents: {
 ...
 * }
 */
export const initConfig = (options:IConfig<typeof gestures>) =>{
  config = options;
}

const capitalize = (str: string): RecognizerType => {
...
/**
   ...
   */
const guardDirections = (options: any) => {
...
/**
   ...
   */
const buildEventWithDirections = (eventName: string, directionArray: []) => {
  const f: any = {};
...
/**
 * 自定义手势指令
 * 如果要配置Hammer事件对应的参数(参照hammer官网),可在vHammer之前调用initConfig
 * 
 * 即:
 * import {initConfig, default as CustomHammer} from "@/utils/hammer";
 * initConfig();
 * const vHammer = CustomHammer;
 */
const CustomHammer:ObjectDirective={
